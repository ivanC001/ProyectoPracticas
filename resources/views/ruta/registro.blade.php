    ...
    document.getElementById('formRegistroRuta').addEventListener('submit', function (event) {
        ...
        .catch((error) => {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: error.response?.data?.message || 'Hubo un problema al registrar la ruta.',
            });
        });
    });
    ...
    function editar(id) {
        ...
        $.ajax({
            ...
            success: function(ruta) {
                ...
                $('#fecha_inicio').val(ruta.fecha_inicio); // Se corrige el ID del campo de fecha de salida
                $('#fecha_fin').val(ruta.fecha_fin); // Se corrige el ID del campo de fecha de llegada
                $('#conductor').val(ruta.conductor_id); // Se establece el ID del conductor
                $('#camion').val(ruta.camion_id); // Se establece el ID del camión
        ...
        $.ajax({
            ...
            success: function(camiones) {
                ...
                $.each(camiones, function(index, camion) {
                    camionSelect.append(`<option value="${camion.id}">${camion.placa_tracto} - ${camion.placa_carreto}</option>`); // Se usa el ID
                });
            },
            error: function() {
                Swal.fire({
        ...
        $.ajax({
            ...
            success: function(conductores) {
                ...
                $.each(conductores, function(index, conductor) {
                    conductorSelect.append(`<option value="${conductor.id}">${conductor.nombre} ${conductor.apellido}</option>`); // Se usa el ID
                });
            },
            error: function() {
                Swal.fire({
